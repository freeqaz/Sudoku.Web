...
(function (root) {
    ...
    // Define difficulties by how many squares are
    // given to the player in a new puzzle.
    var DIFFICULTY = {
        "easy": 62,
        "medium": 53,
        "hard": 44,
    ...
    sudoku.generate = function (difficulty, unique) {
        ...
        */
        ...
        for (var si in shuffled_squares) {
            ...
            for (var sii in SQUARES) {
                var squaree = SQUARES[sii];

                if (candidates[squaree].length == 1) {
                    single_candidates.push(candidates[squaree]);
                }
            }
            ...
            if (single_candidates.length >= difficulty &&
                ...
                for (var ii in SQUARES) {
                    var squareee = SQUARES[ii];
                    if (candidates[squareee].length == 1) {
                        board += candidates[squareee];
                        givens_idxs.push(ii);
                    } else {
                ...
                if (nr_givens > difficulty) {
                    ...
                    for (var iii = 0; iii < nr_givens - difficulty; ++iii) {
                        var target = parseInt(givens_idxs[iii]);
                ...
                if (sudoku.solve(board) && sudoku.hasSomeSpaces(board)) {
                    return board;
                }
            }
        }
    ...
    sudoku.hasSomeSpaces = function (board) {
        // Board doesn't have any blank spaces...
        if (board.indexOf('.') === -1) {
            return false;
        }
        return true;
    ...
    sudoku._search = function (candidates, reverse) {
        ...
        */
        ...
        for (si in SQUARES) {
            var squareeee = SQUARES[si];

            var nr_candidatess = candidates[squareeee].length;

            if (nr_candidatess < min_nr_candidates && nr_candidatess > 1) {
                min_nr_candidates = nr_candidatess;
                min_candidates_square = squareeee;
            }
        }
        ...
        if (!reverse) {
            ...
            for (var vii = min_candidates.length - 1; vii >= 0; --vii) {
                var vall = min_candidates[vii];

                // TODO: Implement a non-rediculous deep copy function
                var candidates_copyy = JSON.parse(JSON.stringify(candidates));
                var candidates_nextt = sudoku._search(
                    sudoku._assign(candidates_copyy, min_candidates_square, vall),
                    reverse
                );

                if (candidates_nextt) {
                    return candidates_nextt;
                }
            }
        }
